...
st.title("Chomsky Normal Form Converter")

terminals = st.text_input("Enter the terminals (separated by spaces):")
# terminals = terminals.split()
variables = st.text_input("Enter the variables (separated by spaces):")
# variables = variables.split()
productions = st.text_area("Enter the productions (one per line):")
# productions = productions.splitlines()

if(st.button("Convert")):
    K, V, Productions = helper.cleanAlphabet(terminals),helper.cleanAlphabet(variables), helper.cleanProduction(productions)
    ...
    # print(Productions)
    stringi = (helper.prettyForm(Productions)).replace("->", "→")
    # print (stringi) but with proper line breaks
     
    st.write(stringi.replace("\n", "<br>", 1000), unsafe_allow_html=True)